...
contract TakeOffToken_test is Context, ERC20Burnable, Ownable {
    ...
    uint256 public _tTotal = 500000000000 * 10 ** _decimals;
    uint256 public _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _tFeeTotal;
    uint256 public _totalSupply = _tTotal;
    uint public latestEthBalance;   //will store the current Eth balance after converting the half balance of the contract 

    string public constant _name = "TAKEOFFTOKEN";
    string public constant _symbol = "TOFF";
    ...
    uint256 public numTokensSellToAddToLiquidity = 3000000 * 10**1 * 10**9;
   ...
   function setMaxPercent(uint256 maxPercent) external onlyOwner() {
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 currentRate) public pure returns (uint256, uint256, uint256) {
    ...
    function restoreAllFee(bool _afterLiquidity) public {
    ...
    function isPossibleLiquidityAdd() public view returns (bool) {
    ...
    function setFee(uint256 multiplier) public {
        _previousLiquidityFee = _liquidityFee;
        _liquidityFee = _liquidityFee+multiplier;
    }

    function swapAndLiquify(uint256 contractTokenBalance) external lockTheSwap {
        // for test cases we are commenting this line because we can't create a call from smart contract.
    ...
    function swapTokensForEth(uint256 tokenAmount, uint256 _minEthAmount) public {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, uint256 _minTokenAmount, uint256 _minEthAmount) public {
        // add the liquidity
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee, bool _afterLiquidity) public {
        if(!takeFee)
            removeAllFee();
